...
class AttendanceRepository {
	async AttendanceInsert(File) {
		try {
			...
			for (let i = 0; i < sheets.length; i++) {
				...
				const sheetData = render.utils.sheet_to_json(readFile.Sheets[sheetname]);
			...
			// console.log(err);
			throw APIError("API Error", STATUS_CODES.INTERNAL_ERROR, "Unable to Create Attendance");
		}
	}
	...
	async getPayroll(id) {
		try {
			...
			throw new APIError("API Error", STATUS_CODES.INTERNAL_ERROR, "Unable to Find Payroll");
		}
	}
	...
	async getSalarySlipByMonthAndyear(id, month, year) {
		try {
			...
			throw new APIError("API Error", STATUS_CODES.INTERNAL_ERROR, "Unable to Find slip");
		}
	}
	...
	async getAttendanceCreate(userId, salaryDetail, month, year) {
		try {
			...
			if (
				attendanceDetail.month === "January" ||
				"March" ||
				"May" ||
				"July" ||
				"August" ||
